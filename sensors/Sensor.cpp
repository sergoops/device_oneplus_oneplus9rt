...
SingleTapSensor::SingleTapSensor(int32_t sensorHandle, ISensorsEventCallback* callback)
    ...
    mSensorInfo.typeAsString = "co.aospa.sensor.single_tap";
    mSensorInfo.maxRange = 2048.0f;
    mSensorInfo.resolution = 1.0f;
    mSensorInfo.power = 0;
    mSensorInfo.flags |= SensorFlagBits::WAKE_UP;
    ...
    mPollFd = open("/sys/devices/platform/soc/990000.i2c/i2c-7/7-004b/single_tap_pressed", O_RDONLY);
    if (mPollFd < 0) {
        ALOGE("failed to open poll fd: %d", mPollFd);
    }
...
DoubleTapSensor::DoubleTapSensor(int32_t sensorHandle, ISensorsEventCallback* callback)
    ...
    mSensorInfo.typeAsString = "co.aospa.sensor.double_tap";
    mSensorInfo.maxRange = 2048.0f;
    mSensorInfo.resolution = 1.0f;
    mSensorInfo.power = 0;
    mSensorInfo.flags |= SensorFlagBits::WAKE_UP;
    ...
    mPollFd = open("/sys/devices/platform/soc/990000.i2c/i2c-7/7-004b/double_tap_pressed", O_RDONLY);
    if (mPollFd < 0) {
        ALOGE("failed to open poll fd: %d", mPollFd);
    }
